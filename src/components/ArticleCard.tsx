      ...
      >
        ...
        <div className="relative flex-shrink-0 w-20 h-20 rounded-lg bg-gray-100 dark:bg-zinc-800 overflow-hidden">
          {article.thumbnail ? (
            ...
            <div
              ...
              style={{ backgroundColor: blog ? `${blog.color}18` : "#f3f4f6" }}
            >
              {blog && (
                <Image src={blog.logoUrl} alt={blog.name} width={32} height={32} className="opacity-30" />
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          <div className="flex items-center gap-1.5">
            {blog && (
              <Image src={blog.logoUrl} alt={blog.name} width={14} height={14} className="rounded-sm" />
            )}
            <span className="text-xs font-semibold" style={{ color: blog?.color ?? "#6b7280" }}>
              {blog?.name ?? article.blog_id}
            </span>
          ...
          {article.tags.length > 0 && (
            <div className="flex gap-1 mt-0.5">
              {article.tags.slice(0, 4).map((tag) => (
                <span key={tag} className="text-xs text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-zinc-800 px-2 py-0.5 rounded">
                  #{tag}
                </span>
              ))}
            </div>
    ...
    >
      ...
      <div className="relative w-full h-44 bg-gray-100 dark:bg-zinc-800 overflow-hidden">
        {article.thumbnail ? (
          ...
          >
            {blog && (
              <Image src={blog.logoUrl} alt={blog.name} width={64} height={64} className="opacity-30" />
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2.5 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            {blog && (
              <Image src={blog.logoUrl} alt={blog.name} width={18} height={18} className="rounded-sm" />
            )}
            <span className="text-xs font-semibold" style={{ color: blog?.color ?? "#6b7280" }}>
              {blog?.name ?? article.blog_id}
            </span>
